...
Motion is available for one explicit workspace and API key. Sneup uses fixed-host GET-only cursor pagination for project and task metadata, keeps only redacted titles, opaque IDs, schedule state, dates, durations, and scheduling-issue flags, and excludes descriptions, creators, assignees, emails, labels, custom fields, embedded project or workspace data, and provider writes.
