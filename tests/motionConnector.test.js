...
describe('Motion connector', () => {
  test('reads bounded workspace project and task metadata without private payload fields', async () => {
    ...
    expect(result.records).toEqual(expect.arrayContaining([expect.objectContaining({ id: 'project:project_1', name: 'Launch [redacted email] [redacted url]' }), expect.objectContaining({ id: 'task:task_1', projectId: 'project_1', priority: 'high', durationMinutes: 90, assigneeIds: ['user_1'], schedulingIssue: true })]));
    ...
    expect(result.metadata.contentPolicy).toContain('opaque_task_assignee_ids_only_for_explicit_capacity_mapping');
  });
  ...
  test('registers Motion as an approval-gated, read-only live adapter', () => {
    ...
    expect(adapter.normalize(account, { id: 'task:task_1', sourceType: 'task', taskId: 'task_1', projectId: 'project_1', name: 'Ship', priority: 'high', assigneeIds: ['user_1'], schedulingIssue: true })).toMatchObject({ externalId: 'task:task_1', description: '', url: undefined, owners: [], labels: expect.arrayContaining(['motion', 'task', 'project:project_1', 'scheduling_issue']), raw: { assigneeIds: ['user_1'] } });
  });
});
