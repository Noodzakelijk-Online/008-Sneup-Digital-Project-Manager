...
| --- | --- | --- | --- | --- |
| ENH-001 | P0 | Connectors | Linked accounts now have bounded read-only API clients for 22 providers. Teamwork adds one-tenant project/task metadata with HTTPS tenant validation, cursor lookback, configured caps, and no private tasks, descriptions, comments, files, time, company, or billing data; Job Health retains provider sync evidence. | Add the next credential-backed provider client, preserving bounded read-only sync and provider-specific health evidence. |
