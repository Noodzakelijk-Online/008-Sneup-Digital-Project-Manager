...
| ENH-005 | P1 | Forecasting | Sneup now models workspace-scoped weekly capacity, allocation, focus time, planned time off, skills, historical card effort, bounded Harvest and Everhour tracked-time utilization, mapped Float/Resource Guru scheduling, and mapped Google Workspace/Microsoft 365 organizer metadata to return analysis-only P50/P80 ranges with confidence, assumptions, and delivery risks. Human-confirmed Float/Resource Guru project IDs map to one exact board, are audit-recorded, reject cross-board duplication, and surface board-scoped scheduling evidence without reducing capacity. Calendar evidence excludes event text, attendees, locations, all-day/cancelled/overlong events, and overlapping-time double counting. | Tune capacity and meeting-load thresholds from reviewed production evidence before considering any calibrated, explicitly approved capacity-reduction model. |
