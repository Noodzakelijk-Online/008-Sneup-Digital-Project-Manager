...
| --- | --- | --- | --- | --- |
| ENH-001 | P0 | Connectors | Linked accounts have 70 bounded read-only API clients. n8n adds public-HTTPS active workflow and execution-health metadata with private-network validation, collection caps, no workflow definitions/credentials/execution data, and no provider writes; Job Health retains provider sync evidence. | Add the next credential-backed provider client, preserving bounded read-only sync and provider-specific health evidence. |
