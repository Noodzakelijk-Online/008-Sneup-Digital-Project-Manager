...
| --- | --- | --- | --- | --- |
| ENH-001 | P0 | Connectors | Linked accounts have 71 bounded read-only API clients. Everhour adds GET-only bounded recent time-entry utilization metadata with no descriptions, notes, budgets, expenses, invoices, rates, profile discovery, or provider writes; n8n retains public-HTTPS workflow and execution-health metadata with private-network validation and collection caps. | Add the next credential-backed provider client, preserving bounded read-only sync and provider-specific health evidence. |
