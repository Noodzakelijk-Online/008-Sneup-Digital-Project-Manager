...
| --- | --- | --- | --- | --- |
| ENH-001 | P0 | Connectors | Linked accounts now have bounded read-only API clients for 21 providers. Coda adds explicit-document allowlisted table metadata without row values, page content, packs, or provider writes; Job Health retains provider sync evidence. | Add the next credential-backed provider client, preserving bounded read-only sync and provider-specific health evidence. |
