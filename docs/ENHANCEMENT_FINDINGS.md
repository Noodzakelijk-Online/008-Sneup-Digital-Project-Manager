...
| ENH-020 | P2 | Resource | The command center now loads only the active view after the initial security context, avoiding the prior fan-out into hidden ledger, connector, signal, forecast, report, and workspace surfaces. Explicit refreshes and workspace changes invalidate cached data. | Add bounded response-timing telemetry for live workspace views. |
