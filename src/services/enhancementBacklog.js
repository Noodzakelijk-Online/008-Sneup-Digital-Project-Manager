...
const enhancements = [
  {
    ...
    evidence: 'Connector accounts can be linked and stored, normalized WorkSignal records now exist, `/api/work-signals/contracts` exposes adapter contracts, first-wave read-only adapters exist for Trello, Jira, Asana, Slack, GitHub, Google Workspace, and Microsoft 365, Trello API-key accounts ingest boards and cards, Jira OAuth accounts discover a single authorized Cloud site before ingesting issues, Asana OAuth accounts select one workspace before ingesting project tasks and dependency context, Slack OAuth accounts ingest bounded accessible channel history with no message-posting scope, GitHub OAuth accounts ingest repository issues and pull requests, Google Workspace OAuth accounts ingest Calendar events plus Drive metadata, and Microsoft OAuth accounts ingest Calendar summaries, To Do task metadata, and signed-in-user OneDrive root metadata through provider APIs using decrypted in-process credentials only, bounded request/item limits, cursor lookback, retry/pacing controls, and no provider writes. A scheduled connector work-signal job records provider retry/pacing evidence in Job Health.',
