...
const enhancements = [
  {
    ...
    evidence: 'Connector accounts can be linked and stored, normalized WorkSignal records now exist, and `/api/work-signals/contracts` exposes adapter contracts. Sneup currently has 70 read-only credential-backed adapters across work management, delivery, communication, resourcing, incident response, CRM, and automation. Each live client decrypts credentials only in process, enforces provider-specific request/item limits and cursor lookback where metadata permits it, performs no provider writes, and records scheduled sync retry/pacing evidence in Job Health. The Generic REST API and n8n adapters reject private-network targets, redirects, oversized responses, raw payload retention, and pagination guessing; n8n retains only bounded active-workflow and execution metadata, never workflow definitions, credentials, or execution data.',
