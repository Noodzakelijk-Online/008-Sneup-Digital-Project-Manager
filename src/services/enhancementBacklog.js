...
const enhancements = [
  {
    ...
    evidence: 'Connector accounts can be linked and stored, normalized WorkSignal records now exist, and `/api/work-signals/contracts` exposes adapter contracts. Sneup currently has 107 read-only credential-backed adapters across work management, delivery, communication, resourcing, incident response, CRM, and automation. Each live client decrypts credentials only in process, enforces provider-specific request/item limits and cursor lookback where metadata permits it, performs no provider content writes, and records scheduled sync retry/pacing evidence in Job Health. Scheduled passes use a bounded two-workspace pool by default (maximum four when explicitly configured), while one process-wide provider queue serializes the full pacing and retry cycle for each provider across workspaces. Tableau Cloud signs in with a personal access token against one allowlisted pod, reads capped project and workbook metadata with GET, invalidates the short-lived session, and excludes descriptions, views, dashboards, data sources, owners, permissions, URLs, tags, and content. Businessmap requires one public HTTPS *.kanbanize.com account URL and API key, reads bounded active boards and paginated active cards with GET only, and excludes descriptions, comments, custom fields, files, dependencies, users, time data, and workflow configuration. TeamGantt requires one explicit company ID and personal access token, reads bounded projects and paginated tasks with GET only, and excludes descriptions, comments, checklists, resources, time blocks, and custom fields. Paymo uses an API key only for bounded active-project and task metadata, excluding descriptions, comments, files, people, billing, budgets, rates, clients, time entries, URLs, and provider writes. Zapier requires a published public integration and `zap:all`, then reads one capped automation page with GET while excluding steps, inputs, linked authentications, editor URLs, runs, user profiles, and webhooks. Jira Align accepts only one HTTPS *.jiraalign.com tenant and user-created API token, then reads capped API v2 portfolio and program metadata without expansion while excluding descriptions, people, custom fields, dependencies, work items, planning details, and URLs. Make accepts a scenarios:read token, one numeric team ID, and only an allowlisted regional host, then retains bounded scenario metadata without blueprints, modules, connections, webhooks, or execution data. The Generic REST API and n8n adapters reject private-network targets, redirects, oversized responses, raw payload retention, and pagination guessing; Everhour retains only bounded recent time-entry utilization metadata, never descriptions, notes, budgets, expenses, invoices, rates, or profile data.',
    impact: 'Turns the connector marketplace from account linking into cross-tool project ingestion.',
    effort: 'XL',
    status: 'in-progress',
    nextStep: 'Add the next credential-backed provider client, preserving bounded read-only sync and provider-specific health evidence. Tableau Cloud now provides capped project and workbook metadata through a short-lived PAT session; prioritize another catalog-only provider with a stable, documented read-only API.',
    acceptanceCriteria: [
