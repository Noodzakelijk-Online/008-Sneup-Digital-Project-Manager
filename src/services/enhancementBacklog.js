...
const enhancements = [
  {
    ...
    evidence: 'Connector accounts can be linked and stored, normalized WorkSignal records now exist, `/api/work-signals/contracts` exposes adapter contracts, first-wave read-only adapters exist for Trello, Jira, Asana, Slack, GitHub, Google Workspace, and Microsoft 365, Trello API-key accounts ingest boards and cards, Jira OAuth accounts discover a single authorized Cloud site before ingesting issues, Asana OAuth accounts select one workspace before ingesting project tasks and dependency context, and GitHub OAuth accounts ingest repository issues and pull requests through provider APIs using decrypted in-process credentials only, bounded request/item limits, cursor lookback, retry/pacing controls, and no provider writes, and a scheduled connector work-signal job records provider retry/pacing evidence in Job Health.',
    impact: 'Turns the connector marketplace from account linking into cross-tool project ingestion.',
    effort: 'XL',
    status: 'in-progress',
    nextStep: 'Add a credential-backed Slack client, preserving bounded read-only sync and provider-specific health evidence.',
    acceptanceCriteria: [
