...
const enhancements = [
  {
    ...
    evidence: 'Connector accounts can be linked and stored, normalized WorkSignal records exist, and `/api/work-signals/contracts` exposes adapter contracts. Sneup currently has 108 read-only credential-backed adapters across work management, delivery, communication, resourcing, incident response, CRM, and automation. Each live client decrypts credentials only in process, enforces provider-specific request and item limits with cursor lookback where metadata permits it, performs no provider content writes, and records scheduled sync retry and pacing evidence in Job Health. Scheduled passes use a bounded two-workspace pool by default (maximum four when explicitly configured), while one process-wide provider queue serializes the full pacing and retry cycle for each provider across workspaces. Kantata OX uses an administrator-registered OAuth application to read bounded workspace project metadata from a fixed endpoint, redacting email addresses and URLs in titles while excluding stories, people, schedules, resources, budgets, financials, attachments, comments, custom fields, provider URLs, and writes. The Generic REST API and n8n adapters reject private-network targets, redirects, oversized responses, raw payload retention, and pagination guessing; Everhour retains only bounded recent time-entry utilization metadata, never descriptions, notes, budgets, expenses, invoices, rates, or profile data.',
