...
const enhancements = [
  {
    ...
    evidence: 'Connector accounts can be linked and stored, normalized WorkSignal records now exist, and `/api/work-signals/contracts` exposes adapter contracts. Sneup currently has 41 read-only credential-backed adapters: Trello; Jira Software/Service Management; Asana; Slack; GitHub; GitLab; Google Workspace; Microsoft 365; Linear; Notion; monday.com; ClickUp; Azure DevOps; Wrike; Smartsheet; Airtable; Todoist; Shortcut; Bitbucket; Harvest; Coda; Teamwork; Basecamp; Redmine; Microsoft Planner; YouTrack; Taiga; Backlog; Freedcamp; MeisterTask; Aha!; Productboard; Toggl Track; Clockify; Float; Resource Guru; Sentry; PagerDuty; Atlassian Statuspage; and a bounded Generic REST API collection adapter. Each live client decrypts credentials only in process, enforces provider-specific request/item limits and cursor lookback where metadata permits it, performs no provider writes, and records scheduled sync retry/pacing evidence in Job Health. The generic adapter also rejects private-network targets, redirects, oversized responses, raw payload retention, and pagination guessing.',
