...
const enhancements = [
  {
    ...
    evidence: 'Connector accounts can be linked and stored, normalized WorkSignal records now exist, `/api/work-signals/contracts` exposes adapter contracts, first-wave read-only adapters exist for Trello, Jira, Asana, Slack, GitHub, Google Workspace, Microsoft 365, Linear, Notion, monday.com, ClickUp, Azure DevOps, Wrike, Smartsheet, Airtable, Todoist, and Shortcut. Trello API-key accounts ingest boards and cards, Jira OAuth accounts discover a single authorized Cloud site before ingesting issues, Asana OAuth accounts select one workspace before ingesting project tasks and dependency context, Slack OAuth accounts ingest bounded accessible channel history with no message-posting scope, GitHub OAuth accounts ingest repository issues and pull requests, Google Workspace OAuth accounts ingest Calendar events plus Drive metadata, Microsoft OAuth accounts ingest Calendar summaries, To Do task metadata, and signed-in-user OneDrive root metadata, Linear OAuth accounts ingest issue context, Notion OAuth accounts ingest only explicitly shared page and data-source metadata, monday.com OAuth accounts ingest board-item metadata using only the `boards:read` scope, ClickUp OAuth accounts ingest bounded workspace task metadata while discarding task descriptions before graph storage, Azure DevOps PAT accounts execute bounded WIQL reads then retrieve only selected work-item fields and relation metadata, Wrike, Smartsheet, Airtable, Todoist, and Shortcut accounts ingest bounded metadata-only work signals without provider writes. Every live client uses decrypted in-process credentials only, bounded request/item limits, cursor lookback, retry/pacing controls, and no provider writes. A scheduled connector work-signal job records provider retry/pacing evidence in Job Health.',
